...
class BukuController extends Controller
{
    ...
    public function listPopularBooks()
    {
        // Ambil ID buku dengan rating tertinggi
        $topRatedBookIds = Rating::select('buku_id', DB::raw('AVG(rating) as average_rating'))
            ->groupBy('buku_id')
            ->orderByDesc('average_rating')
            ->take(10)
            ->pluck('buku_id');

        // Ambil data buku berdasarkan ID yang memiliki rating tertinggi
        $popularBooks = Buku::with('ratings')
            ->whereIn('id', $topRatedBookIds)
            ->orderByDesc('created_at') // Sesuaikan dengan kolom yang sesuai untuk mengurutkan buku
            ->paginate(5);

        $no = 5 * ($popularBooks->currentPage() - 1);

        return view('buku.popular', compact('popularBooks', 'no'));
    }
